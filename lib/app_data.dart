import 'dart:math';
import 'dart:ui';

import 'package:flutter/services.dart';

class AppData {
  static final String appTitle = "Guess my number";
  static final int minRandomNumber = 1;
  static final int maxRandomNumber = 100;
  static final String iAmThinkingText = "I'm thinking of a number between $minRandomNumber and $maxRandomNumber.";
  static final String yourTurnText = "It's your turn to guess my number!";
  static final String tryNumberText = "Try a number!";
  static final String guessButtonText = "Guess";
  static final String restartText = "Restart";
  static final String textFieldHintText = "Enter a number";
  static final String tryHigher = "Try higher";
  static final String tryLower = "Try lower";
  static final String congratulationsText = "Congratulations!";
  static final Color primaryColor = Color(0xff8ABAD3);
  static final Color secondaryColor = Color(0xffEDC2D8);
  static final Color silverColor = Color(0xffAAA9AD);
  static final Color whiteColor = Color(0xffFCF6F5);
  static int randomGeneratedNumber = generateRandomNumberBetween(minRandomNumber, maxRandomNumber);
  static int userSelectedNumber = -2;
  static String cardText = tryNumberText;
  static String buttonText = guessButtonText;

  // method for removing 0 if it's the first character and the input has more than one character
  static String handleZeroFirstCharacter(String input) {
    return input.substring(1);
  }

  // method for generating a random integer between min and max
  static int generateRandomNumberBetween(int min, int max) {
    return new Random().nextInt(max) + min;
  }

  // method for verifying if the user's number is equal to the random generated number
  static int checkIfTheInputNumberIsEqualToTheRandomGeneratedNumber(int number) {
    int outputNumber = 0;

    if (number != AppData.randomGeneratedNumber)
      outputNumber = number < AppData.randomGeneratedNumber ? -1 : 1;

    return outputNumber;
  }

  // method for closing the app
  static void closeApp()
  {
    SystemChannels.platform.invokeMethod('SystemNavigator.pop');
  }
}
